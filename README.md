...
2. **터키 소비자물가지수(CPI) 3년 장기 추이** — FRED(`TURCPIALLMINMEI`)에서 월별 CPI를 가져와
   YoY / MoM 상승률과 복합 차트(꺾은선+막대)로 표시합니다. API 실패 시 더미 데이터로 대체됩니다.
...
│   ├── cpi_data.py              # FRED 터키 CPI(TURCPIALLMINMEI) 조회 및 YoY/MoM 계산
...
### `API 처리 지연` / 429 메시지가 1분 후에도 계속될 때

Gemini 무료 티어는 **분당 요청 수(RPM)** 와 **일일 사용량** 한도가 있습니다.

- 예전에는 429가 난 뒤에도 새로고침할 때마다 API를 다시 호출해, 제한이 더 길어지거나 일일 한도를 빨리 소진할 수 있었습니다.
- 지금은 429가 나면 **약 3분(분당 제한)** 또는 **약 1시간(일일 한도 의심)** 동안 API를 다시 치지 않고 안내만 보여줍니다.
- 화면에 **일일 사용량 초과** 안내가 보이면, 1~3분만 기다려서는 해결되지 않습니다. 다음날 다시 시도하거나 [Google AI Studio](https://aistudio.google.com/)에서 사용량/플랜을 확인해 주세요.
- 연속 새로고침은 상황을 악화시키므로 피하고, 꼭 필요할 때만 **지금 다시 시도** 버튼을 사용하세요.
...
- 터키 CPI는 FRED `TURCPIALLMINMEI` 시리즈를 `pandas-datareader`로 조회하며, 하루(`ttl=86400`)에
  한 번 갱신합니다. 네트워크/API 실패 시 최근 3년 물가 추이(대략 YoY 40%~80%)를 반영한 더미
  데이터로 자동 대체됩니다.
